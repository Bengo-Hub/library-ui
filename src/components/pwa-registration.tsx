...
export function PWARegistration() {
  ...
  return (
    <div
      className="fixed inset-x-0 bottom-0 z-50 flex justify-center px-4 animate-slide-in-right sm:inset-x-auto sm:right-4 sm:justify-end"
      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-3xl border border-border/60 bg-card/95 shadow-2xl shadow-black/25 ring-1 ring-black/5 backdrop-blur-xl">
        ...
        <div className="flex items-start gap-3.5 px-4 pt-4 pb-3">
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            ...
            <p className="text-xs text-muted-foreground mt-1 leading-snug">
              {ios ? 'Add to Home Screen for offline access.' : 'Browse the catalog offline — syncs when reconnected.'}
            </p>
          </div>
          <button
            onClick={dismiss}
        ...
        {ios ? (
          ...
          <div className="flex items-center gap-2 px-4 pb-4">
            <Button size="sm" variant="ghost" className="flex-1 text-muted-foreground" onClick={dismiss}>
              Later
            </Button>
            <Button
              size="sm"
